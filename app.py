...
# --- Fonction utilitaire pour lire CSV sans problème d'encodage ---
def read_csv_safe(uploaded_file):
    """
    Lit un CSV uploadé en gérant tous les problèmes d'encodage.
    """
    try:
...
if file_users and file_entreprises and file_mises_relation and file_base_globale:
    ...
    df_users = read_csv_safe(file_users)
    df_entreprises = pd.read_excel(file_entreprises)
    df_mises = read_csv_safe(file_mises_relation)
    df_globale = pd.read_excel(file_base_globale)

    # --- Nettoyage colonnes ---
    df_users.columns = df_users.columns.str.strip()
    df_entreprises.columns = df_entreprises.columns.str.strip()
    df_mises.columns = df_mises.columns.str.strip()
    df_globale.columns = df_globale.columns.str.strip()
    ...
    df_users["Date de dernière connexion"] = pd.to_datetime(
        df_users["Date de dernière connexion"], dayfirst=True, errors="coerce"
    )
